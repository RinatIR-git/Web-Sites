...
public partial class DeleteUsers : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            st += "<table border = '1'><tr><td>שם פרטי</td><td>שם משפחה </td><td>מייל</td><td>סיסמא</td><td>מגדר</td><td>סניף</td><td>מי אני?</td><td>מחיקה</td>";
            st += "</tr>";
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                st += "<td>" + "<a href=delete.aspx?mail=" + dt.Rows[i]["email"] + ">מחק</a>";
                st += "</tr>";
            }
            st += "</table>";
        }
